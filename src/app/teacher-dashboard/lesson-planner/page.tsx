...
const LessonPlannerPage = () => {
  ...
  const handleGenerateFlashcards = async (topic: string) => {
    try {
      ...
      toast({
        ...
        description: "AI has generated flashcards.",
      });
    } catch (e: any) {
      setError(e.message);
  ...
  // Chart data
  const chartData =
    lessonPlanData?.lessonPlan.map(item => ({
      name: `Week ${item.week}`,
  ...
  const handleDeleteLessonPlan = async (planId: string) => {
    if (!user) return;
    try {
      await deleteDoc(doc(db, `teachers/${user.uid}/lessonPlans`, planId));
    ...
    <div className="container mx-auto py-8">
      <Card className="max-w-5xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Define goals and dates to generate a plan.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          ...
          {lessonPlanData && (
            <div className="grid gap-4">
              ...
              <p>
                <strong>Intended Outcomes:</strong>{" "}
                {lessonPlanData.intendedOutcomes}
              </p>
              ...
              <div className="mb-4 border p-4 rounded">
                <ScrollArea className="h-[400px] w-full rounded-md border">
                  {lessonPlanData.lessonPlan.map(
                    (item: LessonPlanItem, idx: number) => (
                      <div key={idx} className="mb-4 border p-4 rounded">
                        <Button
                          className="mt-2"
                          onClick={() =>
                            handleGenerateFlashcards(item.topic)
                          }
                        >
                          Generate Flashcards
                        </Button>
      ...
      >
        <DialogContent className="max-w-4xl">
          ...
          <DialogFooter>
            <Button
              variant="secondary"
              onClick={() => setOpenViewPlans(false)}
            >
              Close
            </Button>
          </DialogFooter>
      ...
      {/* View Single Plan */}
      <Dialog
        open={!!selectedLessonPlan}
        onOpenChange={() => setSelectedLessonPlan(null)}
      >
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              {selectedLessonPlan?.lessonTitle}
            </DialogTitle>
            <DialogDescription>
              Details of the selected lesson plan.
            </DialogDescription>
          </DialogHeader>
          {selectedLessonPlan && (
            <div className="grid gap-4">
              ...
              <p>
                ...
                {selectedLessonPlan.gradeLevel}
              </p>
              <p>
                <strong>Learning Objectives:</strong>{" "}
                {selectedLessonPlan.learningObjectives}
              </p>
              <ScrollArea className="h-[400px] w-full rounded-md border">
                {selectedLessonPlan.lessonPlan.map(
                  (item: LessonPlanItem, idx: number) => (
                    <div key={idx} className="mb-4 border p-4 rounded">
                      <h3 className="text-lg font-semibold">
                        Week {item.week}
